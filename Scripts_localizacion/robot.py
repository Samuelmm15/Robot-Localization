...
# Hay que normalizar la orientación entre pi y -pi
...
# Esto no es real, es una aproximación
  def pose(self):
    # Obtener pose actual
    return [self.x, self.y, self.orientation]

# Para cada una de la balizas se calcula su distancia y se añade un ruido
...
# El robot se puede mover de dos formas
...
# Esta es la otra forma
  ...
  def measurement_prob(self, measurements, landmarks):
    ...
    for i in range(len(measurements)-1):
      ...
      n=n+1
    # Si esta diferencia es muy grande, indica que el robot se ha desviado mucho
    # Si esta diferencia es pequeña o muy cercana a cero, indica que el robot se ha desviado poco o nada
